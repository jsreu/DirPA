...
def test_classtaskdataset_task_iter() -> None:
    num_classes = 3
    total_classes = 10
    task_ds = interval_tasks(
        total_classes=total_classes, num_classes=num_classes, sample=False
    )
...
def test_split_data(train_samples: int | None) -> None:
    ...
    train_ix, test_ix = split_data(
        ixs, dataset, train_samples=train_samples, test_samples=10
    )
...
def test_split_data_insufficient_data() -> None:
    ...
    with pytest.raises(ValueError):
        train_ix, test_ix = split_data(
            ixs, dataset, train_samples=None, test_samples=200
        )
