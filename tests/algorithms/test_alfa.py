...
def test_alfa_inner_update(alfa: ALFA, tasks: list[Task]) -> None:
    ...
    recorded_params: list[dict[str, dict[str, float]]] = (
        alfa.inner_learner.recorded_params  # type: ignore[assignment]
    )
    for p1, p2, g in zip(
    ...
    for p1, p2, g in zip(
        model.head.parameters(), model_copy.head.parameters(), head_grads
    ):
        expected = (
            recorded_params[0]["head"]["beta"] * p2
            - recorded_params[0]["head"]["alpha"] * g
        )
        assert torch.allclose(p1, expected)
...
def test_alfa_adapt(alfa: ALFA, tasks: list[Task]) -> None:
    ...
    alfa.adapt(tasks)
    for p1, p2 in zip(
        alfa.model.backbone.parameters(), old_model.backbone.parameters()
    ):
...
def test_alfa_eval(alfa: ALFA, tasks: list[Task]) -> None:
    ...
    assert {metric.name for metric in metrics} == {
        metric.name for metric in task.metrics
    }
