...
def test_anil_adapt(anil: ANIL, tasks: list[Task]) -> None:
    ...
    anil.adapt(tasks)
    for p1, p2 in zip(
        anil.model.backbone.parameters(), old_model.backbone.parameters()
    ):
    ...
    inner_lr = anil.inner_learner.lr.item()  # type: ignore[operator]
...
def test_anil_eval(anil: ANIL, tasks: list[Task]) -> None:
    ...
    for p1, p2 in zip(
        anil.model.backbone.parameters(), task_model.backbone.parameters()
    ):
        assert (p1 == p2).all()
    ...
    assert inner_lr == anil.inner_learner.lr.item()  # type: ignore[operator]
