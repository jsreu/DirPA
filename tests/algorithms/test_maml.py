...
def test_maml_adapt(maml: MAML, tasks: list[Task]) -> None:
    ...
    maml.adapt(tasks)
    for p1, p2 in zip(
        maml.model.backbone.parameters(), old_model.backbone.parameters()
    ):
    ...
    inner_lr = maml.inner_learner.lr.item()  # type: ignore[operator]
...
def test_maml_eval(maml: MAML, tasks: list[Task]) -> None:
    ...
    assert inner_lr == maml.inner_learner.lr.item()  # type: ignore[operator]
