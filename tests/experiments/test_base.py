...
def test_tuned_experiment_run_tuning(
    ...
    tuning_result = mock_experiment.run_tuning(
        study_name=study_name, tuning_params=tuning_params
    )
    ...
    for n, call_arg in enumerate(mlflow_logger_mock.log_json.call_args_list):
        ...
        assert mlflow_logger_mock.start_run.call_args_list[n].args == (
            logged_result.run_name,
        )
...
def test_tuned_experiment_run_training(
    ...
    training_result = mock_experiment.run_training(
    ...
    loaded_run_results = get_run_results(
        runs_dir=mock_experiment.runs_dir, prefix="training"
    )
...
def test_tuned_experiment_load_run_results(
    ...
    run_result = RunResult(
        ...
        config={
            ...
            for name, config in mock_experiment.run_config().items()
        },
    )
