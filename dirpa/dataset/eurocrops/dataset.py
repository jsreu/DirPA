...
class EuroCropsDataset(Dataset[LabelledData]):
    ...
    def _format_dates(
        ...
        for satellite, dates in arrays_dict["dates"].items():
            arrays_dict["dates"][satellite] = pd.to_datetime(dates).day_of_year.to_numpy() - 1
        return arrays_dict
    ...
    def __getitem__(
        ...
        if self.config.normalize:
            ...
            if "S2" in np_data_dict:
                nodata_mask = np_data_dict["S2"] == -9999
                ...
                # clip extreme outliers to keep the distribution stable
                normalized_s2[nodata_mask] = 0.0
                np_data_dict["S2"] = normalized_s2
