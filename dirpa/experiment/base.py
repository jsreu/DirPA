...
from dirpa.experiment.logger import MLFlowCallback, MLFlowLogger
from dirpa.experiment.runs import RunResult, get_run_results
from dirpa.experiment.tuning import run_tuning
from dirpa.experiment.utils import (
    TuningConfig,
    get_model_builder,
    get_timestamp,
    overwrite_config,
)
from dirpa.models.base import Model, ModelConfig
from dirpa.settings import Settings
from dirpa.train.callback import (
    OptunaCallback,
    TrainCallback,
    ValidationCallback,
)
from dirpa.train.train import TrainConfig, Trainer
from dirpa.train.utils import set_seed
...
class ExperimentConfig(BaseModel):
    ...
    warmup_steps: The minimum number of training steps/epochs that must be completed before the
    best-performing model checkpoint can be saved or the best metric can be updated. This prevents
    selection bias towards models that achieve highly volatile, but high, metric scores very early
    in training (e.g., due to aggressive hyperparameter sampling)), promoting the selection of runs
    that exhibit long-term stability and sustained generalization.
    If 0, the first validation run is eligible for saving/selection.
    """
    ...
    def _wrapped_run(
        ...
        try:
            ...
            validation_callback = ValidationCallback(
                key_metric=self.config.key_metric, warmup_steps=self.config.warmup_steps
            )
